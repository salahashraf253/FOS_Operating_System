...
void * create_page_table(uint32 *ptr_page_directory, const uint32 virtual_address)
{
	...
	uint32 pagetable = (uint32) kmalloc(PAGE_SIZE);
		uint32 phys_page_table=kheap_physical_address(pagetable);
		ptr_page_directory[PDX(virtual_address)] =
				CONSTRUCT_ENTRY(phys_page_table, PERM_PRESENT | PERM_USER | PERM_WRITEABLE);
		tlbflush();
